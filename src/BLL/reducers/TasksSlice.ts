...
export const tasksSlice = createSlice({
  ...
  reducers: {
    ...
    deleteTask: (state, action: PayloadAction<DeleteTask>) => {
      state.tasks[action.payload.todolistId] = state.tasks[action.payload.todolistId].filter(
        t => t.id !== action.payload.taskId
      )
    },
    updateTask: (state, action: PayloadAction<UpdateType>) => {
      state.tasks[action.payload.todolistId] = state.tasks[action.payload.todolistId].map(
        (t: TaskType) =>
          t.id === action.payload.taskId ? { ...t, ...action.payload.updateModel } : t
      )
    },
  },
})
...
export const { addTodosByTasks, getTasks, addTask, deleteTask, deleteTasksByTodo, updateTask } =
  tasksSlice.actions
